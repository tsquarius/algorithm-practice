...
function minChange(coins, amount, memo = {}) {
  ...
  let usable = coins.filter(coin => coin <= amount);
  let min;

  for (let i in usable) {
    let count = 1 + minChange(coins, amount - usable[i], memo);
    if (!min) min = count;
...
function minChange2(coins, amount, memo = {}) {
  if (amount in memo) return memo[amount];
  if (amount === 0) return 0;
  let min;
  ...
  for (let i in coins) {
    ...
    let count = 1 + minChange2(coins, amount - coins[i], memo);
    if (!min) min = count;
    if (min > count) min = count;
  }
